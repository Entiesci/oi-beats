...
1. 对于每个环，在环心建一个方点。这个**方点是不存在于原图的**。让环上一个割点做环的根，从割点向方点连一条权值为0的边，从方点向其它圆点连一条权值为s的边，s是圆点到割点的最短路。这个方点与环上其它圆点连成菊花图。
...
这样，图上的最短路转化为树上的最短路，我们先用倍增法找lca(u,v)，设其为p。然后分类讨论:
（1）若p为圆点，那答案就是树上这两点的距离$d[u]+d[v]-d[p]*2$。
（2）若p为方点，则需要找出p在环上的两个儿子A,B，分别是u和v的祖先。答案应该为$d(A,B)+d(A,u)+d(B,v)$。
$d(A,B)$可以利用环长信息求解，len = abs(s[A]-s[B]); d(A,B) = min(len,sc[A]-len)。其中，s[A]是A到割点的环长，sc[A]是A所在环的总长度。
